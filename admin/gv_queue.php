  ...
  if ($HTTP_GET_VARS['action']=='confirmrelease' && isset($HTTP_GET_VARS['gid'])) {
    ...
    if ($gv_result['release_flag']=='N') {
      ...
      if ($customer_gv) {
        ...
        $gv_update=tep_db_query("update " . TABLE_COUPON_GV_QUEUE . " set release_flag='Y', date_released = NOW() where unique_id='".$HTTP_GET_VARS['gid']."'");
      }
    }
  }
...
<?php
  ...
  switch ($HTTP_GET_VARS['action']) {
    case 'release':
      ...
      $contents[] = array('align' => 'center', 'text' => tep_draw_button(IMAGE_CONFIRM, 'folder-open',tep_href_link('gv_queue.php','action=confirmrelease&gid='.$gInfo->unique_id,'NONSSL')) . tep_draw_button(IMAGE_CANCEL, 'cancel',tep_href_link('gv_queue.php','action=cancel&gid=' . $gInfo->unique_id,'NONSSL')));
      break;
    default:
      $heading[] = array('text' => '[' . $gInfo->unique_id . '] ' . tep_datetime_short($gInfo->date_created) . ' ' . $currencies->format($gInfo->amount));

      $contents[] = array('align' => 'center','text' =>  tep_draw_button(IMAGE_RELEASE, 'folder-open',tep_href_link('gv_queue.php','action=release&gid=' . $gInfo->unique_id,'NONSSL')) . '</a>');
      break;
   }
