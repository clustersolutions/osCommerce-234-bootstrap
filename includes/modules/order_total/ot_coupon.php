...
global $PHP_SELF, $order, $currencies;
  ...
  if ($od_amount > 0) {
    ...
    $this->output[] = array('title' => $this->title . ' "' . $this->coupon_code .'": ',
                            'text' => '<b><font color=green>- ' . $currencies->format($od_amount) . '</b>',
                            'value' => $od_amount);
  }
}
...
function credit_selection() {
 ...
 // $selection_string .= tep_draw_button(IMAGE_REDEEM_VOUCHER, 'refresh', null, 'primary');
...
function collect_posts() {
  ...
  global $cart, $fb_likes, $HTTP_POST_VARS, $customer_id, $currencies, $cc_id, $messageStack;
  ...
  if ($HTTP_POST_VARS['gv_redeem_code']) {
    ...
    if ($coupon_result['coupon_type'] != 'G') {
      ...
      $start_date_query=tep_db_query("select coupon_start_date from " . TABLE_COUPONS . " where coupon_start_date <= now() and coupon_code='".$HTTP_POST_VARS['gv_redeem_code']."'");

      $expire_date_query=tep_db_query("select coupon_expire_date from " . TABLE_COUPONS . " where coupon_expire_date >= now() and coupon_code='".$HTTP_POST_VARS['gv_redeem_code']."'");
      ...
      if (tep_db_num_rows($coupon_query)==0) {
        $gv_error = true;
        $messageStack->add('payment_error', ERROR_NO_INVALID_REDEEM_COUPON);
        //tep_redirect(tep_href_link(FILENAME_CHECKOUT_CONFIRMATION, 'payment_error='.$this->code.'&error=' . urlencode(ERROR_NO_INVALID_REDEEM_COUPON), 'SSL'));
      } elseif (tep_db_num_rows($start_date_query)==0) {
        $gv_error = true;
        $messageStack->add('payment_error', ERROR_INVALID_STARTDATE_COUPON);
        //tep_redirect(tep_href_link(FILENAME_CHECKOUT_CONFIRMATION, 'payment_error='.$this->code.'&error=' . urlencode(ERROR_INVALID_STARTDATE_COUPON), 'SSL'));
      } elseif (tep_db_num_rows($expire_date_query)==0) {
        $gv_error = true;
        $messageStack->add('payment_error', ERROR_INVALID_FINISDATE_COUPON);
        //tep_redirect(tep_href_link(FILENAME_CHECKOUT_CONFIRMATION, 'payment_error='.$this->code.'&error=' . urlencode(ERROR_INVALID_FINISDATE_COUPON), 'SSL'));
      } elseif (tep_db_num_rows($coupon_count)>=$coupon_result['uses_per_coupon'] && $coupon_result['uses_per_coupon'] > 0) {
        $gv_error = true;
        $messageStack->add('payment_error', ERROR_INVALID_USES_COUPON);
        //tep_redirect(tep_href_link(FILENAME_CHECKOUT_CONFIRMATION, 'payment_error='.$this->code.'&error=' . urlencode(ERROR_INVALID_USES_COUPON . $coupon_result['uses_per_coupon'] . TIMES ), 'SSL'));
      } elseif (tep_db_num_rows($coupon_count_customer)>=$coupon_result['uses_per_user'] && $coupon_result['uses_per_user'] > 0) {
        $gv_error = true;
        $messageStack->add('payment_error', ERROR_INVALID_USES_USER_COUPON);
        //tep_redirect(tep_href_link(FILENAME_CHECKOUT_CONFIRMATION, 'payment_error='.$this->code.'&error=' . urlencode(ERROR_INVALID_USES_USER_COUPON . $coupon_result['uses_per_user'] . TIMES ), 'SSL'));
      }
      ...
      if (!$gv_error) {
        ...
        if ( strlen($cc_id) > 0 && $coupon_amount == 0 ) {
          if ($this->get_order_total() == $order->info['subtotal']) {
            $messageStack->add('payment_error', ERROR_INVALID_USES_COUPON);
          } else {
            $messageStack->add('payment_error', ERROR_REDEEMED_AMOUNT_ZERO . ($coupon_result['coupon_minimum_order'] > 0 ? ERROR_PROMO_CODE_MIN . $currencies->format($coupon_result['coupon_minimum_order']) : ''));
          }
          //$err_msg = ERROR_REDEEMED_AMOUNT_ZERO;
        } /*else {
  	  //$err_msg = ERROR_REDEEMED_AMOUNT . ': ' . $coupon_amount_out;
        }*/
        //tep_redirect(tep_href_link(FILENAME_CHECKOUT_CONFIRMATION, 'payment_error='.$this->code.'&error=' . urlencode($err_msg), 'SSL'));
      } else {
 ...
 // if ($HTTP_POST_VARS['submit_redeem_coupon_x']) tep_redirect(tep_href_link(FILENAME_CHECKOUT_CONFIRMATION, 'payment_error='.$this->code.'&error=' . urlencode(ERROR_NO_REDEEM_CODE), 'SSL'));
}

function calculate_credit($amount) {
global $cart, $customer_id, $order, $cc_id, $fb_likes;
  $od_amount = 0;
  if (isset($cc_id) ) {
		...
		if ($pr_ids[$ii] == tep_get_prid($order->products[$i]['id'])) {
		  if ($get_result['coupon_type'] == 'P') {
		    ...
		    $pod_amount = round($pr_c*10)/10*$c_deduct/100;
                    if (tep_get_products_special_price($order->products[$i]['id'])) $pod_amount = 0; //Cluster Solutions added to exclude special price 071114
          ...
          if ($get_result['coupon_type'] !='P') {
            ...
            $qty = array(); //Cluster Solutions added to excluded multi qty special price products. 1122014
            $products = $cart->get_products();
            for ($i=0; $i<sizeof($products); $i++) {
              $qty[tep_get_prid($products[$i]['id'])] = $cart->get_quantity($products[$i]['id']);
            }

            $od_amount = $amount * $get_result['coupon_amount'] / 100;
            foreach ($cart->get_product_specials_id_list() as $val) { // Cluster Solutions added to exclude special price 071114
              $od_amount =  $od_amount - tep_get_products_special_price($val) * $get_result['coupon_amount'] / 100 * $qty[$val]; 
            }
          }
        }
      }
...
global $customer_id, $order, $cc_id, $cart, $fb_likes;
  $coupon_query = tep_db_query("select coupon_code from " . TABLE_COUPONS . " where coupon_id = '" . $cc_id . "'");
  if (tep_db_num_rows($coupon_query) !=0 ) {
    $coupon_result = tep_db_fetch_array($coupon_query);
    $coupon_get = tep_db_query("select coupon_amount, coupon_minimum_order, restrict_to_products, restrict_to_categories, coupon_type from " . TABLE_COUPONS . " where coupon_code = '". $coupon_result['coupon_code'] . "'");
      ...
      if ($get_result['coupon_type'] != 'S') {
        if ($get_result['restrict_to_products'] || $get_result['restrict_to_categories']) {
          ...
          for ($i=0; $i<sizeof($products); $i++) {
            ...
            if ($get_result['restrict_to_products']) {
              $pr_ids = preg_split("/[,]/", $get_result['restrict_to_products']);
              if (!tep_get_products_special_price($t_prid)) { //Cluster Solutions added to exclude special price 071114  
            ...
            if ($method=='Credit Note') {
              ...
              while (list($key, $value) = each($order->info['tax_groups'])) {
                ...
                if ($net>0) {
                  ...
                  foreach ($cart->get_product_specials_id_list() as $val) { // Cluster Solutions added to exclude special price 071114
                    $aa =  $aa + tep_get_products_special_price($val) * $tax_rate / 100 * $get_result['coupon_amount'] / 100 * $qty[$val];
                  }
                  $god_amount = $god_amount - $aa;
                  $tod_amount += $god_amount;
                  $order->info['tax_groups'][$key] = $order->info['tax_groups'][$key] - $god_amount;
                }
              }
...
global $order, $cart, $customer_id, $cc_id;
  ...
  if (tep_db_num_rows($coupon_query) !=0) {
    ...
    if ($get_result['restrict_to_products']) {
      ...
      if ($get_result['coupon_type'] == 'F') {
        if ($total_price > $get_result['coupon_amount']) $total_price = $get_result['coupon_amount'];
        $order_total = $order_total - $total_price; //tmp fix Cluster Solutions 12-06-2014
      } else {
        $order_total = $total_price;
      }
    }
...
function get_error() {
  ...
  $error = array('title' => MODULE_ORDER_TOTAL_COUPON_TEXT_ERROR,
                 'error' => stripslashes(urldecode(html_entity_decode($HTTP_GET_VARS['error']))));
