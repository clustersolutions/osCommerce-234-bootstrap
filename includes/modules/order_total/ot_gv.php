<?php
  ...
  class ot_gv {
    ...
    function ot_gv() {
      ...
      $this->checkbox = '<input type="checkbox" name="' . 'c' . $this->code . '" ' . ($HTTP_POST_VARS['cot_gv'] ? 'checked="checked"' : '') . '>';
      $this->output = array();
    }
       ...
       if (tep_session_is_registered('cot_gv')) {
        ...
        if ($od_amount > 0) {
          $this->output[] = array('title' => $this->title . ':',
                           'text' => '<b><font color=green>- ' . $currencies->format($od_amount) . '</font></b>',
                           'value' => $od_amount);
        }
      }
    }
...
function mod_process() {
  ...
  function update_credit_account($i, $order_id=0) {
	...
	//	  $selection_string .= tep_draw_button(IMAGE_REDEEM_VOUCHER, 'refresh', null, 'primary', array(params=>'onClick="submitFunction();"'));
    ...
    function collect_posts() {
      global $messageStack, $currencies, $HTTP_POST_VARS, $customer_id, $coupon_no, $REMOTE_ADDR, $cot_gv;
      ...
      if ($HTTP_POST_VARS['gv_redeem_code']) {
        ...
        if (tep_db_num_rows($gv_query) != 0) {
          ...
          if ( (tep_db_num_rows($redeem_query) != 0) && ($gv_result['coupon_type'] == 'G') ) {
            ...
            $gv_error = true;
            $messageStack->add('payment_error', ERROR_NO_INVALID_REDEEM_GV);
        ...
        } elseif ($HTTP_POST_VARS['submit_redeem_x']) {
            $messageStack->reset();
            $HTTP_POST_VARS['gv_redeem_code'] = '';
            $gv_error = true;
            $messageStack->add('payment_error', ERROR_NO_INVALID_REDEEM_GV);
        ...
        if ($gv_result['coupon_type'] == 'G'  && !$gv_error) {
          ...
          $HTTP_POST_VARS['gv_redeem_code'] = '';
          //tep_redirect(tep_href_link(FILENAME_CHECKOUT_CONFIRMATION, 'payment_error='.$this->code.'&error=' . urlencode(ERROR_REDEEMED_AMOUNT. $currencies->format($gv_amount)), 'SSL'));
          $messageStack->add('payment_error', ERROR_REDEEMED_AMOUNT. $currencies->format($gv_amount), 'success');
        }
      }
     // if ($HTTP_POST_VARS['submit_redeem_x'] && !$HTTP_POST_VARS['gv_redeem_code']) tep_redirect(tep_href_link(FILENAME_CHECKOUT_CONFIRMATION, 'payment_error='.$this->code.'&error=' . urlencode(ERROR_NO_REDEEM_CODE), 'SSL'));
    }
